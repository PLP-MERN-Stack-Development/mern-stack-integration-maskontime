...
export default function EditPostPage() {
  ...
  const onSubmit = async (e) => {
    ...
    setError('')
    try {
      await postService.update(id, { title, content, image })
      navigate(`/posts/${id}`)
    } catch (e) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 bg-white p-4 rounded shadow">
      ...
      <button disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-60">Save</button>
    </form>
  )
}
